...
# Supabase 客户端初始化：提前捕获密钥缺失或网络异常，避免后续操作直接崩溃
supabase = None
supabase_error = None
try:
    ...
    supabase_error = f"无法初始化数据库客户端：{e}"
...
def load_and_sort_questions():
    # 读取题库并做必要校验：缺文件、解析异常、缺少关键列会直接中断并提示
    try:
    ...
    required_cols = {"stem", "answer"}
    missing_cols = required_cols - set(df.columns)
    if missing_cols:
        st.error(f"❌ 题库缺少必要列：{', '.join(sorted(missing_cols))}")
        st.stop()
...
# --- 在主程序初始化部分加入状态 ---
# processing 用于防重复提交，确保用户点击后界面锁定至提交完成
...
if not st.session_state.submitted:
    ...
    with col1:
        if  st.session_state.processing:
            # 状态 A：处理中，显示一个不可点击的按钮（用 disabled=True）
            st.button("⏳ 提交中...", type="primary", key="btn_processing", disabled=True,on_click=keep_going)  
            st.session_state.submitted = True
            st.rerun()
        else:
            # 状态 B：待提交，显示正常按钮
            if st.button("✅ 提交答卷", type="primary", key="final_confirm"):
                # 用户点击后，立即切换状态并刷新
                st.session_state.processing = True
                st.rerun()
            ...
            #     on_click= keep_going
            # ):
            #     st.session_state.processing = True  # 锁定状态
                # st.rerun()  # 触发重绘

# --- 步骤 4：评分与结果 ---
if st.session_state.submitted:
    # 状态块包裹评分与上传流程，给用户明确的处理反馈
    with st.status("正在核对答案并上传成绩...", expanded=True) as status:
        total_score = 0
        correct_count = 0
        details = []
